...
This page lists all known amendments and their statuses.
...
> **Note**: Error loading amendments:: Failed to fetch


## Amendments in Development

The following is a list of amendments that are being actively developed. They can become open for voting on Mainnet if they are included in a stable server release, and they may already be enabled on other networks.

| Name | Status | Additional Information |
| --- | --- | --- |
| Hooks |  | XRPL Hooks |
| InvariantsV1_1 |  |  |
| SingleAssetVault |  | Single Asset Vault (Ripple Opensource) |


This list is updated manually. If you're working on an amendment and have a private network to test the changes, you can edit this page to add your in-development amendment to this list. For more information on contributing to the XRP Ledger, see Contribute Code to the XRP Ledger.


## Obsolete Amendments

The following is a list of known amendments that have been removed in a previous version of the core server, or are obsolete and have been marked for removal.

| Name | Introduced | Status |
| --- | --- | --- |
| fixNFTokenNegOffer | v1.9.2 |  |
| fixNFTokenDirV1 | v1.9.1 |  |
| NonFungibleTokensV1 | v1.9.0 |  |
| CryptoConditionsSuite | v0.60.0 |  |
| OwnerPaysFee | v0.33.0 |  |
| SHAMapV2 | v0.32.1 |  |
| Tickets | v0.30.1 |  |
| SusPay | v0.31.0 |  |
| FlowV2 | v0.32.1 |  |
...
For details, see the XLS-73: AMMClawback specification.
...
Adjusts the CheckCash transaction so that cashing a Check for an issued token automatically creates a trust line to hold the token. The new behavior is similar to how the OfferCreate transaction behaves when users buy tokens in the decentralized exchange: the automatic trust line has a limit value of 0. This removes the setup step of setting up a trust line before receiving a token via a Check. (Checks that send XRP are unaffected.)
...
See Clawback for details on this amendment.
...
For more details, see the XLS-70: Credentials specification.
...
Although this amendment is enabled, it has no effect unless the SusPay amendment is also enabled. The SusPay amendment was replaced by the Escrow amendment, so the CryptoConditions amendment has no effect.
...
- RippleState flags:lsfLowDeepFreezelsfHighDeepFreeze
- lsfLowDeepFreeze
- lsfHighDeepFreeze
- TrustSet flags:tfSetDeepFreezetfClearDeepFreeze
- tfSetDeepFreeze
- tfClearDeepFreeze
...
With this amendment, new accounts start with their Sequence numbers equal to the Sequence number matching the index of the ledger in which the account is created. This change protects accounts that are deleted and later re-created from having their old transactions executed again. Adds a new AccountDelete transaction type, which deletes an account and certain objects that the account owns in the ledger. Certain types of objects cannot be deleted this way, so an account that is linked to any such objects cannot be deleted. Additionally, an account cannot be deleted if the current ledger index minus 256 is less than the account's current Sequence number. See XRP Community Standards Draft 7 for a detailed discussion of this amendment.
...
1. tfMutable: New flag that enables authorized accounts to modify the URI of an NFT. This flag must be enabled when the NFT is initially minted.
...
- AccountRoot objects in the ledger cannot be deleted unless DeletableAccounts is enabled. (See also: Deleting Accounts.)
- An entry in the ledger cannot change its type. (The LedgerEntryType field is immutable.)
...
- Changes the TrustSet transaction to fail with tecNO_PERMISSION if it tries to enable the No Ripple flag but cannot because the trust line has a negative balance. Without this amendment, the transaction does not enable the No Ripple flag, but the transaction result is tesSUCCESS nonetheless.
...
- Add rounding to AMM deposit and withdraw to ensure that the AMM's balance meets the invariant:On deposit, tokens out are rounded downward and deposit amount is rounded upward.On withdrawal, tokens in are rounded upward and withdrawal amount is rounded downward.
...
1. The user sets asfDisallowIncomingTrustline on their account.
1. The user submits a SetTrust transaction to the issuer.
1. The issuer is unable to authorize the trustline.
...
1. The holder can receive frozen LP tokens, but can't send them out (similar to frozen trust lines).
...
This amendment standardizes the way inner objects (Object-type fields in the canonical binary format) have their formats and default values enforced. This is the same type of check that the fixInnerObjTemplate applies to AMM-related fields, but this amendment applies to all other types of inner objects, namely:

`fixInnerObjTemplate`

- DisabledValidator field of the NegativeUNL entry.
- Members of the Majorities array in the Amendments entry.
- Members of the Signers array of multi-signed transactions.
- Members of the SignerEntries array of SignerList ledger entries.
- Several parts of the XChainBridge amendment :Members of the XChainClaimAttestations array in XChainOwnedClaimID ledger entriesMembers of the XChainCreateAccountAttestations array in XChainOwnedCreateAccountClaimID ledger entriesMembers of the XChainClaimAttestationBatch array in XChainAddClaimAttestation transactionsMembers of the XChainCreateAccountAttestationBatch array in XChainAddClaimAttestation transactions
...
In general, offers can be reduced in 3 ways:
...
Removes the tfTrustLine setting on non-fungible tokens, to protect against a denial of service attack on issuers using this flag. With this amendment enabled, a NFTokenMint transaction with the tfTrustLine flag enabled is considered invalid and cannot be confirmed by consensus; new NFToken objects cannot be minted with the flag.
...
This amendment has no effect unless either NonFungibleTokensV1 or NonFungibleTokensV1_1 is enabled. To protect issuers, this amendment should be enabled before NonFungibleTokensV1 or NonFungibleTokensV1_1.
...
Without this amendment, the code continues to use separate calculations for STAmount and IOUAmount objects, and Automated Market Maker (XLS-30d) uses a third class for calculations.
...
Adds on-chain smart contracts in the form of small pieces of code that can run on an account before or after transactions. For more information, see the Hooks Documentation.
...
Reduces the owner reserve counted against your XRP Ledger account when it owns a multi-signing SignerList.
...
WarningThere are several known issues with this amendment including one that can cause tecINVARIANT_FAILED errors to appear in the ledger. It has been replaced by the NonFungibleTokensV1_1 amendment.
...
Adds native support for non-fungible tokens, including fixes to several issues that were discovered after NonFungibleTokensV1.
...
CautionThe fixRemoveNFTokenAutoTrustLine fixes an known issue with this amendment. When creating a new test network, you should make sure that these amendments should be enabled together or the fix amendment is enabled first.
...
Fixes an inconsistency in the way transfer fees are calculated between OfferCreate and Payment transaction types. Without this amendment, the holder of the token pays the transfer fee if an offer is executed in offer placement, but the initial sender of a transaction pays the transfer fees for offers that are executed as part of payment processing. With this amendment, the holder of the token always pays the transfer fee, regardless of whether the offer is executed as part of a Payment or an OfferCreate transaction. Offer processing outside of payments is unaffected.

This Amendment requires the Flow Amendment to be enabled.

NoteAn incomplete version of this amendment was introduced in v0.33.0 and removed in v0.80.0. (It was never enabled.)
...
This amendment adds a "Price Oracle" feature to the XRP Ledger, as defined in the XLS-47 specification. A blockchain oracle is a system where a service provides to the blockchain about the outside world, which can then be used by decentralized applications (dApps) that run primarily on or using the blockchain. This price oracle is intended to store pricing information about asset pairs that exist outside of the XRP Ledger so that smart contracts that rely on the XRP Ledger can use this information.
...
Changes the signature requirements for the XRP Ledger protocol so that non-fully-canonical signatures are no longer valid in any case. This protects against transaction malleability on all transactions, instead of only transactions with the tfFullyCanonicalSig flag enabled.
...
WarningOlder versions of rippled that do not know about this amendment may crash when they find a DirectoryNode sorted by the new rules. To avoid this problem, upgrade to rippled version 0.80.0 or later.
...
![In Development: TBD](https://img.shields.io/badge/In Development-TBD-lightgrey.svg)

![In Development: TBD](https://img.shields.io/badge/In Development-TBD-lightgrey.svg)

![In Development: TBD](https://img.shields.io/badge/In Development-TBD-lightgrey.svg)

![Obsolete: To Be Removed](https://img.shields.io/badge/Obsolete-To Be Removed-red.svg)
...
![Obsolete: Removed in v0.80.0](https://img.shields.io/badge/Obsolete-Removed in v0.80.0-red.svg)

![Obsolete: Removed in v1.4.0](https://img.shields.io/badge/Obsolete-Removed in v1.4.0-red.svg)

![Obsolete: Removed in v0.90.0](https://img.shields.io/badge/Obsolete-Removed in v0.90.0-red.svg)

![Obsolete: Removed in v0.60.0](https://img.shields.io/badge/Obsolete-Removed in v0.60.0-red.svg)

![Obsolete: Removed in v0.33.0](https://img.shields.io/badge/Obsolete-Removed in v0.33.0-red.svg)

![Image](https://t.co/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=df3a8ba9-d5a6-4c4c-a7ae-296adc822ed5&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o309v&type=javascript&version=2.3.34)

![Image](https://analytics.twitter.com/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=df3a8ba9-d5a6-4c4c-a7ae-296adc822ed5&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o309v&type=javascript&version=2.3.34)

![Image](https://t.co/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=aba36324-8388-4aae-b585-5b0492bae73d&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o61w3&type=javascript&version=2.3.34)

![Image](https://analytics.twitter.com/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=aba36324-8388-4aae-b585-5b0492bae73d&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o61w3&type=javascript&version=2.3.34)

![Image](https://bat.bing.com/action/0?ti=26326193&Ver=2&mid=c840a515-5cd6-41a6-824c-eda2be5ab486&bo=1&sid=9c430e509dac11f098800bf801021da7&vid=9c43c6409dac11f0bd3e2f2bd38c272c&vids=1&msclkid=N&uach=pv%3D10.0&pi=0&lg=en-US@posix&sw=1280&sh=720&sc=24&nwd=1&tl=Known%20Amendments&p=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&r=&lt=3721&evt=pageLoad&sv=2&cdb=AQAS&rn=253324)

![Image](https://t.co/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=014c8355-6936-478c-bbdd-a71e3a57a6e2&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o309v&type=javascript&version=2.3.34)

![Image](https://analytics.twitter.com/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=014c8355-6936-478c-bbdd-a71e3a57a6e2&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o309v&type=javascript&version=2.3.34)

![Image](https://t.co/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=aef56b06-9993-4eb5-b8b0-fa36baa02f0d&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o61w3&type=javascript&version=2.3.34)

![Image](https://analytics.twitter.com/i/adsct?bci=3&dv=UTC%26en-US%40posix%26Google%20Inc.%26Linux%20x86_64%26255%261280%26720%264%2624%261280%26720%260%26na&eci=2&event_id=aef56b06-9993-4eb5-b8b0-fa36baa02f0d&events=%5B%5B%22pageview%22%2C%7B%7D%5D%5D&integration=advertiser&p_id=Twitter&p_user_id=0&pl_id=59f07761-1095-433e-b0f4-a1108ecb30b9&pt=Known%20Amendments&tw_document_href=https%3A%2F%2Fxrpl.org%2Fresources%2Fknown-amendments&tw_iframe_status=0&tw_order_quantity=0&tw_sale_amount=0&txn_id=o61w3&type=javascript&version=2.3.34)
...
- [About](https://xrpl.org/resources/known-amendments#)
...
- [Docs](https://xrpl.org/resources/known-amendments#)
...
- [Resources](https://xrpl.org/resources/known-amendments#)
...
- [Community](https://xrpl.org/resources/known-amendments#)
...
- [Edit](https://github.com/XRPLF/xrpl-dev-portal/tree/master/resources/known-amendments.md)
- [[Source]](https://github.com/XRPLF/rippled/blob/develop/include/xrpl/protocol/detail/features.macro)
- [Single Asset Vault (Ripple Opensource)](https://opensource.ripple.com/docs/xls-65d-single-asset-vault)
- [XLS-30](https://github.com/XRPLF/XRPL-Standards/tree/master/XLS-0030-automated-market-maker#readme)
- [XLS-73: AMMClawback specification](https://github.com/XRPLF/XRPL-Standards/discussions/212)
- [XLS-56](https://github.com/XRPLF/XRPL-Standards/tree/master/XLS-0056-batch)
- [XLS-70: Credentials specification](https://github.com/XRPLF/XRPL-Standards/tree/master/XLS-0070-credentials)
- [crypto-conditions specification](https://tools.ietf.org/html/draft-thomas-crypto-conditions-03)
- [XRP Community Standards Draft 7](https://github.com/XRPLF/XRPL-Standards/issues/8)
- [World Wide Web Consortium](https://www.w3.org/press-releases/2022/did-rec/)
- [Interledger Protocol Crypto-Conditions](https://tools.ietf.org/html/draft-thomas-crypto-conditions-02)
- [issue #4925](https://github.com/XRPLF/rippled/issues/4925)
- [issue #4941](https://github.com/XRPLF/rippled/issues/4941)
- [PR #4945](https://github.com/XRPLF/rippled/pull/4945)
- [well known reference implementation in JavaScript](https://gist.github.com/N3TC4T/a20fb528931ed009ebdd708be4938748?permalink_comment_id=4738760#gistcomment-4738760)
- [PR 4346](https://github.com/XRPLF/rippled/pull/4346)
...
- [Automated Market Maker (XLS-30d)](https://github.com/XRPLF/XRPL-Standards/discussions/78)
- [Hooks Documentation](https://xrpl-hooks.readme.io/)
- [XLS-20d](https://github.com/XRPLF/XRPL-Standards/discussions/46)
- [Interledger Protocol](https://interledger.org/)
- [XLS-75](https://github.com/XRPLF/XRPL-Standards/tree/master/XLS-0075-permission-delegation)
- [XLS-81](https://github.com/XRPLF/XRPL-Standards/pull/281)
- [XLS-47 specification](https://github.com/XRPLF/XRPL-Standards/blob/master/XLS-0047-PriceOracles/README.md)
- [rippled issue #3042](https://github.com/XRPLF/rippled/issues/3042)
- [XLS-65](https://github.com/XRPLF/XRPL-Standards/tree/master/XLS-0065d-single-asset-vault)
- [XLS-13d](https://github.com/XRPLF/XRPL-Standards/issues/16)
- [XLS-85](https://github.com/XRPLF/XRPL-Standards/pull/272/)
- [XLS-0038 Cross-Chain Bridge](https://github.com/XRPLF/XRPL-Standards/blob/master/XLS-0038-cross-chain-bridge/README.md)
- [XRPL Foundation](https://xrpl.foundation)
- [XRPL Grants](https://xrplgrants.org/)
- [GitHub](https://github.com/XRPLF/)

---
Crawled on: 2025-09-30T03:23:43.032Z
