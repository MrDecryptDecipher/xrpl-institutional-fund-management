...
async function testRealXRPLIntegration() {
  ...
  // Define mptId at the right scope
  const mptId = 'MPT_TEST_ID';
  ...
  try {
    ...
    const didDocument = {
      ...
      verificationMethod: [{
        id: `did:xrpl:testnet:${issuerWallet.address}#key-1`,
        type: "Ed25519VerificationKey2020",
        controller: `did:xrpl:testnet:${issuerWallet.address}`,
        publicKeyMultibase: issuerWallet.publicKey
      }],
      authentication: [`did:xrpl:testnet:${issuerWallet.address}#key-1`],
      assertionMethod: [`did:xrpl:testnet:${issuerWallet.address}#key-1`]
    };
    ...
    const prepared3 = await client.autofill(domainSetTransaction as any);
    ...
    if (result3.result.validated) {
      ...
      const domainCreatedNode = (result3.result.meta as any)?.CreatedNode;
      const domainId = domainCreatedNode?.NewFields?.DomainID || 
                      `DOM${result3.result.hash?.substring(0, 16).toUpperCase()}`;
      console.log(`   Domain ID: ${domainId}`);
    } else {
      throw new Error(`PermissionedDomainSet failed: ${(result3.result.meta as any)?.TransactionResult}`);
    }
    
    // Test 4: LoanBrokerSet (XLS-65/66) - Create Lending Pool
    console.log("\n🧪 Test 4: Creating Lending Pool (XLS-65/66)");
    const loanBrokerSetTransaction = {
    ...
    const prepared4 = await client.autofill(loanBrokerSetTransaction as any);
    const signed4 = issuerWallet.sign(prepared4);
    ...
    if (result4.result.validated) {
      ...
      throw new Error(`LoanBrokerSet failed: ${(result4.result.meta as any)?.TransactionResult}`);
    }
    ...
    const authorizationTransaction = {
      ...
      Holder: investorWallet.address
      // Note: In a real implementation, additional fields would be added based on XLS-33 specification
    };
    ...
    console.log(`   Found ${accountObjects.result.account_objects.length} MPT objects`);
    
    const mptObject = accountObjects.result.account_objects.find(
      (obj: any) => obj.MPTokenIssuanceID === mptId
    );
    
    if (mptObject) {
      console.log(`   MPT Balance: ${(mptObject as any).MPTAmount}`);
    } else {
      console.log("   MPT object not found in account objects");
    }
